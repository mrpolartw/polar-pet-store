...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(err?.body?.message || err?.message || '申請失敗，請稍後再試')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="auth-page">
      <div className="auth-brand-panel">
        ...
        <div className="auth-brand-content">
          <h2>忘記密碼？<br />我們幫你找回</h2>
          <p>輸入註冊 Email，我們會寄出重設密碼連結。重設連結有效時間為 15 分鐘。</p>
        </div>
        <div className="auth-brand-features">
          <div className="auth-brand-feature">
            <div className="auth-brand-feature-icon"><Mail size={16} /></div>
            <span>重設密碼連結會寄送到你的信箱</span>
          </div>
        </div>
      </div>
          ...
          {successMessage ? (
            ...
            <form className="auth-form" onSubmit={handleSubmit} noValidate>
              ...
              <button type="submit" className="btn-blue auth-submit-btn" disabled={isLoading}>
                {isLoading ? <><span className="auth-spinner" />送出中...</> : '寄送重設連結'}
              </button>
