  ...
  useEffect(() => {
    ...
    const confirm = async () => {
      try {
        ...
        if (!isMounted) return
        setStatus(STATUS.INVALID)
        setMessage(error?.body?.message || error?.message || '驗證失敗，請重新申請驗證信')
      }
    }
  ...
  return (
    <main className="reset-password-page">
      ...
      <div className="reset-password-card">
        <div className={`reset-password-icon ${isSuccess ? 'reset-password-icon--success' : 'reset-password-icon--error'}`}>
          {isSuccess ? <CheckCircle2 size={32} strokeWidth={1.5} /> : <XCircle size={32} strokeWidth={1.5} />}
        </div>
        <h1 className="reset-password-title">
          {status === STATUS.SUCCESS && 'Email 驗證成功'}
          {status === STATUS.ALREADY && 'Email 已驗證'}
          {status === STATUS.EXPIRED && '驗證連結已過期'}
          {status === STATUS.USED && '驗證連結已失效'}
